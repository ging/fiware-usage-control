...
if [ ! -z "$ADVERTISED_HOST" ] && [ ! -z "$ADVERTISED_PORT" ]; then
    ...
    if grep -q "^advertised.listeners" $KAFKA_HOME/config/server.properties; then
        sed -r -i "s/#(advertised.listeners)=PLAINTEXT://$ADVERTISED_HOST:$ADVERTISED_PORT/g" $KAFKA_HOME/config/server.properties
    else
    ...
    if grep -q "^listeners" $KAFKA_HOME/config/server.properties; then
        sed -r -i "s/#(listeners)=PLAINTEXT://$ADVERTISED_HOST:$ADVERTISED_PORT/g" $KAFKA_HOME/config/server.properties
    else
        echo "listeners=PLAINTEXT://$ADVERTISED_HOST:$ADVERTISED_PORT" >> $KAFKA_HOME/config/server.properties
    fi
fi
